...
struct IpoTotView: View {
    
    @Environment(\.presentationMode) var presentationMode: Binding<PresentationMode>
    
    @Binding var scrollTopTot: Bool
    
    @State private var ActiveIpoTotToIpoDetail: Bool = false
    ...
    @State private var ipoData: IpoTotData?
        ...
        ZStack {
            VStack (spacing: 0) {
                ScrollViewReader { scrollProxy in
                        ...
                        Section {
                            ...
                            ForEach(ipoData?.공모주 ?? [], id: \.종목코드) { item in
                                ...
                                ZStack {
                                    ...
                                    .sheet(isPresented: $ActiveIpoTotToIpoDetail) {
                                        IpoDetailView(
                                            구분: $구분,
                                            종목코드: $종목코드,
                                            종목명: $종목명
                    ...
                    .onChange(of: scrollTopTot) { newValue in
                        print(newValue)
                        withAnimation {
    ...
    func fetchData() {
        isRefreshing = true
        guard let url = URL(string: "http://idghstrpa.dothome.co.kr/api/공모주_전체.php?count=999") else {
            return
        }
        URLSession.shared.dataTask(with: url) { data, response, error in
            if let data = data {
                if let decodedData = try? JSONDecoder().decode(IpoTotData.self, from: data) {
...
struct IpoTotView_Previews: PreviewProvider {
    static var previews: some View {
        IpoTotView(scrollTopTot: .constant(false))
